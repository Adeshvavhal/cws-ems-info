...
function AllEmployeeRegularizationRequestForAdmin({ showBackButton = true }) {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axios.get("https://cws-ems-backend.vercel.app/attendance/regularization/all", {
  ...
  const handleStatusChange = async (id, status) => {
    try {
      const token = localStorage.getItem("accessToken");
      await axios.put(
        `https://cws-ems-backend.vercel.app/attendance/regularization/${id}/status`,
        { status },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      fetchData();
