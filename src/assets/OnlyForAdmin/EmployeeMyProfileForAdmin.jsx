    ...
    if (!stateEmployee) {
      const fetchEmployee = async () => {
        try {
          ...
          const res = await axios.get(`https://cws-ems-backend.vercel.app/getEmployee/${empId}`, {
...
case "uanNumber":
  ...
  if (new Date(dob) > minDate) {
    ...
    try {
      ...
      await axios.put(`https://cws-ems-backend.vercel.app/employees/${employee._id || empId}`, data, {
      ...
      const updated = await axios.get(`https://cws-ems-backend.vercel.app/getEmployee/${employee._id || empId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
...
const getDocumentPreview = (fileName, label) => {
  ...
  const url = fileName.startsWith("http")
    ? fileName
    : `https://cws-ems-backend.vercel.app/uploads/${fileName}`;
...
const handlePermanentDelete = async (id) => {
  ...
  try {
    const token = localStorage.getItem("accessToken");
    const res = await axios.delete(`https://cws-ems-backend.vercel.app/deleteEmployee/${id}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
                ...
                <img
                  src={
                    employee?.image?.startsWith("http")
                      ? employee.image
                      : `https://cws-ems-backend.vercel.app/uploads/${employee.image}`
                  }
                  alt="Profile Preview"
                  style={{
                    width: "100px",
            ...
            <div className="d-flex gap-3 flex-wrap">
                    ...
                    <a
                      href={
                        employee[field]?.startsWith("http")
                          ? employee[field]
                          : `https://cws-ems-backend.vercel.app/uploads/${employee[field]}`
                      }
                      target="_blank"
              ...
              <div className="d-flex flex-column align-items-start">
                ...
                {isEditing ? (
                  ...
                  <a
                    href={`https://cws-ems-backend.vercel.app/uploads/${employee.bankDetails.passbookPdf}`}
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                    <img
