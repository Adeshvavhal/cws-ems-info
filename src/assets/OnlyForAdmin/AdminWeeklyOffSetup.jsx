...
function AdminWeeklyOffSetup() {
  ...
  useEffect(() => {
    const fetchWeeklyOff = async () => {
      try {
        const res = await axios.get(`https://cws-ems-backend.vercel.app/admin/weeklyoff/${year}`);
  ...
  const saveWeeklyOff = async () => {
    try {
      await axios.post("https://cws-ems-backend.vercel.app/admin/weeklyoff", { year, saturdays });
