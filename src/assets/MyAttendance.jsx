 ...
  }, [workMode]);
  ...
  useEffect(() => {
    if (selectedRecord?.leaveRef?.reportingManager) {
      axios
        .get(`https://cws-ems-backend.vercel.app/users/${selectedRecord.leaveRef.reportingManager}`)
    ...
    const fetchData = async () => {
      try {
        const [attRes, leaveRes, weeklyRes, holidayRes, regRes] = await Promise.all([
          axios.get(`https://cws-ems-backend.vercel.app/attendance/${employeeId}`),
          axios.get(`https://cws-ems-backend.vercel.app/leave/my/${employeeId}`),
          axios.get(`https://cws-ems-backend.vercel.app/admin/weeklyoff/${new Date().getFullYear()}`),
          axios.get(`https://cws-ems-backend.vercel.app/getHolidays`),
          axios.get(`https://cws-ems-backend.vercel.app/attendance/regularization/my/${employeeId}`),
        ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const authAxios = axios.create({
        baseURL: "https://cws-ems-backend.vercel.app",
