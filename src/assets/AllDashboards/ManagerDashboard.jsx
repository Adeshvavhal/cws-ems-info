    ...
    const fetchData = async () => {
      try {
        const leavesRes = await axios.get(
          `https://cws-ems-backend.vercel.app/leaves/manager/${user._id}`
        );
        setLeaves(leavesRes.data);

        const regRes = await axios.get(
          `https://cws-ems-backend.vercel.app/regularization/manager/${user._id}`
        );
...
//bg scroll stop
  ...
  const updateLeaveStatus = async (leaveId, status) => {
    try {
      const res = await axios.put(
        `https://cws-ems-backend.vercel.app/leave/${leaveId}/status`,
        {
          status,
          userId: user._id,
          role: "manager",
  ...
  const updateRegularizationStatus = async (attendanceId, status) => {
    try {
      const res = await axios.put(
        `https://cws-ems-backend.vercel.app/attendance/regularization/${attendanceId}/status`,
        {
          status,
          approvedBy: user._id,
