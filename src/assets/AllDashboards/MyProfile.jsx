...
function MyProfile({ user }) {
  ...
  const handleRemoveImage = async (e) => {
    ...
    try {
      ...
      const res = await axios.delete(
        `https://cws-ems-backend.vercel.app/employees/${user._id}/image`,
      );
    ...
    const fetchProfile = async () => {
      try {
        const res = await axios.get(
          `https://cws-ems-backend.vercel.app/employees/${user._id}`,
        );
        setProfile(res.data);
        setFormData({
          ...res.data,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axios.put(`https://cws-ems-backend.vercel.app/employees/${user._id}`, data, {
  ...
  const renderFileLink = (file) => {
    ...
    return (
      <a
        href={`https://cws-ems-backend.vercel.app/uploads/${file}`}
        target="_blank"
        rel="noreferrer"
      >
        View PDF
                  ...
                  <img
                    src={
                      profile.image?.startsWith("http")
                        ? profile.image
                        : `https://cws-ems-backend.vercel.app/image/${profile.image}`
                    }
                    alt="Profile"
                    style={{
                      width: "100px",
